  ...
  describe "POST #create" do
    context "with invalid params" do
      it "validates the presence of the user's email and password"
        post :create, user: {email: "noahskang@gmail.com", password: ""}
        expect(response).to render_template("new")
        expect(flash[:errors]).to be_present

      it "validates that the password is at least 6 characters long"
        post :create, user: {email: "noah@gmail.com", password: "asdf"}
        expect(response).to render_template("new")
        expect(flash[:errors]).to be_present
      end
    end
    ...
    context "with valid params" do
      it "redirects user to bands index on success"
        post :create, user: {email: "noah@gmail.com", paassword: "asdfasdf"}
