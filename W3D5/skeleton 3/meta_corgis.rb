...
class MetaCorgiSnacks
  ...
  #   tastiness = @snack_box.("get_#{name}_tastiness", @box_id)
  #   name = "#{name.to_s.split('_').map(&:capitalize).join(' ')}"
  #   result = "#{name}: #{info} #{tastines} "
  #   tastiness > 30 ? "* #{result}" : result
  # end
  ...
  def self.define_snack(name)
    define_method(name)
      info = @snack_box.send("get_#{name}_info, @box_id"))
      # this send is going to call the method named "get_name_info"(@box_id)
      # the variable info is going to contain a pointer to a particular box, with a snack and its type
      tastiness = @snack_box.send("get_#{name}_tastiness, @box_id")
      # the variable info is going to contain a pointer to a particular box, with a snack and its tastiness
      result = "#{name}: #{info}: #{tastiness} "
      # this will be a string that we will output when this method is called if tastiness is < 30. 
      tastiness > 30 ? "* #{result}" : result
    end
  end
end
